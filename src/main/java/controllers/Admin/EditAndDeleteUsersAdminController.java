...
public class EditAndDeleteUsersAdminController {
    ...
    public void initialize() {
        ...
        // --- Configuración de Filtrado y Ordenación ---

        // 2. Envolver la ObservableList principal en una FilteredList
        FilteredList<UsuarioInfoDTO> filteredData = new FilteredList<>(masterUsersData, p -> true);

        // 3. Añadir un Listener al TextField para aplicar el filtro
        tfFilter.textProperty().addListener((observable, oldValue, newValue) -> {
            filteredData.setPredicate(user -> {
                ...
                // Comparar el nombre del usuario con el texto del filtro
                String lowerCaseFilter = newValue.toLowerCase();

                // Filtra por la columna 'name' (nombre)
                if (user.getName().toLowerCase().contains(lowerCaseFilter)) {
                    return true; // Coincide
                }

                // Opcional: Podrías añadir más campos aquí, como 'username' o 'email'
        ...
        // 4. Envolver la FilteredList en una SortedList para mantener la ordenación
        // (ya que FilteredList no es modificable y no mantiene la capacidad de ordenar por columna)
        SortedList<UsuarioInfoDTO> sortedData = new SortedList<>(filteredData);

        // 5. Vincular el comparador de SortedList al comparador de TableView.
        // Esto asegura que la ordenación de la tabla (al hacer clic en las cabeceras)
        // se aplique a los datos de SortedList y, por lo tanto, a la tabla.
        sortedData.comparatorProperty().bind(usersTable.comparatorProperty());

        // 6. Asignar la SortedList a la TableView
    ...
    private void loadUsersData() {
        ...
        masterUsersData = FXCollections.observableArrayList(users);
    }
